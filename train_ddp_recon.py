...
def train_epoch(epoch, summary, summary_writer, model, loss_fn, optimizer, dataloader_train, cfg):
    ...
    img, target, labels = prefetcher.next()
    for step in range(steps):
        ...
        # print(data.shape)
        output = model(data)
        output = F.softmax(output, dim=1) * 5
        ...
        acc = (sum((target == 0) & (output <= 0.5)) +
               sum((target == 1) & ((output > 0.5) & (output <= 1.5))) +
               sum((target == 2) & ((output > 1.5) & (output <= 2.5))) +
               sum((target == 3) & ((output > 2.5) & (output <= 3.5))) +
               sum((target == 4) & ((output > 3.5) & (output <= 3.5))))/img.size(0)
...
def run():
    ...
    loss_fn = nn.CrossEntropyLoss(
        weight=torch.Tensor([0.5, 0.6, 0.7])).to(device)
